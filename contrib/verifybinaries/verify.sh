...
###   This script attempts to download the signature file SHA256SUMS.asc from chilicoin.org
...
WORKINGDIR="/tmp/chilicoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://chilicoin.org/bin/"
VERSIONPREFIX="chilicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'chilicoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the ChiliCoin Core binary release signing key installed?"
   fi
